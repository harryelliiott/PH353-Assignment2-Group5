...
MC_data = np.zeros((len(beta_list),3)) #Uses the expensive Monte Carlo algorithm to find data for <U>
...
reweight_data = U(x,d,beta, re_beta_list, 20, configurations) #Uses reweighted data
plt.figure()
plt.subplot(1,2,1)
plt.errorbar(MC_data[:,0],MC_data[:,1], yerr = MC_data[:,2], fmt ='b*')
plt.xlabel("Inverse temperature beta")
plt.ylabel("Average energy <U>")
plt.title("Pure Monte Carlo data")
plt.subplot(1,2,2)
plt.errorbar(reweight_data[:,0],reweight_data[:,1],yerr = reweight_data[:,2],fmt ='g*')  
plt.title("Reweighted data")
plt.xlabel("Inverse temperature beta")
plt.ylabel("Average energy <U>")
plt.tight_layout()
plt.savefig("Inverse_temperature_plots.pdf")
plt.close()
